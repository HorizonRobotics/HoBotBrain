...
def main(params: DictConfig):
    ...
    # chair in the living room on floor 0-
    # loop forever and ask for query, until user click 'q'
    while True:
        query = input("Enter query: ")
        if query == "q":
            break
        # query = query.rstrip() + " in room 0 on floor 0\n" 
        print(query)
        # query="sofa in the living room on floor 0"
        start_time = time.time()
        floor, room, obj = hovsg.query_hierarchy(query, top_k=3)
        end_time = time.time()
