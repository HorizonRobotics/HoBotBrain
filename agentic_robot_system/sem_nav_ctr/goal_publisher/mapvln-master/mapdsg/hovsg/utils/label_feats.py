...
def get_label_feats(clip_model, clip_feat_dim, obj_labels, label_feat_path=None):
    ...
    if obj_labels == "COCO_STUFF_CLASSES":
        ...
        # TODO: change this
        label_feat_path = "hovsg/labels"
        classes_matrix = pd.read_csv(os.path.join(label_feat_path, "imagenet21k.csv"), header=0, sep=";")
        classes = list(classes_matrix[classes_matrix.keys()[0]].values)
        text_feats, classes = compute_label_feats(clip_model, clip_feat_dim, label_feat_path, classes, "text_feats_IMAGENET21K_LABELS.npy")
    elif obj_labels == "SCANNET200":
        # TODO: change this
        label_feat_path = "hovsg/labels"
        classes_matrix = pd.read_csv(os.path.join(label_feat_path, "scannet200.csv"), header=0, sep=";")
        classes = list(classes_matrix[classes_matrix.keys()[0]].values)
        text_feats, classes = compute_label_feats(clip_model, clip_feat_dim, label_feat_path, classes, "text_feats_SCANNET200_LABELS.npy")
    elif obj_labels == "SCANNET20":
        # TODO: change this
        label_feat_path = "hovsg/labels"
        classes_matrix = pd.read_csv(os.path.join(label_feat_path, "scannet20.csv"), header=0, sep=";")
        classes = list(classes_matrix[classes_matrix.keys()[0]].values)
        text_feats, classes = compute_label_feats(clip_model, clip_feat_dim, label_feat_path, classes, "text_feats_SCANNET20_LABELS.npy")
    elif obj_labels == "FINALLABEL":
        # TODO: change this
        label_feat_path = "hovsg/labels"
        classes_matrix = pd.read_csv(os.path.join(label_feat_path, "final_label.csv"), header=0, sep=";")
        classes = list(classes_matrix[classes_matrix.keys()[0]].values)
        text_feats, classes = compute_label_feats(clip_model, clip_feat_dim, label_feat_path, classes, "text_feats_FINALLABEL_LABELS.npy")
    return text_feats, classes
